...
export default class ViewDescriptor {
  ...
  public static parseStyleFrom(json: ICytoscapeJSON,
    ...
    const defSelectors = def.map((i) => i.selector);
    json.style.forEach((s) => {
      // Skip style for core, this is generated by cytoscape, not sure it's
      // meaning.
      if (s.selector === "core") {
        return;
      }
      // If the selector is one of the default style, skip it.
      if (defSelectors.indexOf(s.selector) !== -1) {
        return;
      }
